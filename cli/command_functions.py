...
def find_command_list(command):
    # a generic function for creating a list from a given command
    # options are, manual index, playlist,
    pass
        ...
        if "play." in run_command:
            ...
            while shuffle:
                try:
                    ...
                    #
                    #     #make a new randomized list
                    #     randomized_content_list = []
                    #     for number in range(len(content_indexes)):
                    #         random_index = choice(content_indexes)
                    #         content_indexes.remove(random_index)
                    #         randomized_content_list.append(random_index)
                    #         # picked_index = play_random(content_indexes)
                    #         # picked_content = category_contents.content_list[int(picked_index)]
                    #
                    # #randomize the entire category
                    # else:
                    #     randomized_content_list = []
                    #     for number in range(len(category_contents.content_list)):
                    #         random_index = choice(content_indexes)
                    #         content_indexes.remove(random_index)
                    #         randomized_content_list.append(random_index)
                    #
                    #     picked_content = play_next(randomized_content_list)
